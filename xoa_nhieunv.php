...
if (isset($_POST['delete'])) {
    if (!empty($_POST['manv'])) {
        $ma_nhan_vien_list = array_map(function($manv) use ($conn) {
            return "'" . mysqli_real_escape_string($conn, $manv) . "'";
        }, $_POST['manv']);
        $ma_nhan_vien_list = implode(",", $ma_nhan_vien_list);
        
        $sql = "DELETE FROM nhanvien WHERE manv IN ($ma_nhan_vien_list)";
        ...
        echo "<div class='alert error'>Vui lòng chọn ít nhất một nhân viên để xóa.</div>";
    }
}
...
<header>
    ...
    <form method="POST" action="">
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td><input type='checkbox' name='manv[]' value='" . htmlspecialchars($row['manv']) . "'></td>";
                        echo "<td>" . $stt++ . "</td>";
                        ...
                        echo "<td>" . htmlspecialchars($row['mapb']) . "</td>";
                        echo "</tr>";
                    }
                } else {
        ...
        <div class="button-group">
            <button type="submit" name="delete" class="button">Xóa Nhân Viên Đã Chọn</button>
